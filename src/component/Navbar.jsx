...
const Navbar = () => {
  return (
    <>
    <div className='max-w-screen-xl mx-auto py-5 flex items-center  justify-between border-b-2 border-zinc-700'>
        ...
        <div className='links flex gap-14 ml-20'>
            {["Home","Work","Culture","","News"].map((elem,index)=>(
                ...
                <a key={index} className='text-sm flex items-center gap-1 ' href='#'>
                    {index ===1 && (<span key={index} style={{boxShadow:"0 0 0.45em #00FF19"}} className='inline-block rounded-full w-2 h-2 bg-green-400'></span>)}
                {elem}</a>)
            ))}
        </div>
       </div>
